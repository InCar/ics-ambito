...
public abstract class AuthorizeRealm extends AuthenticateRealm{
    ...
    private AuthorizeInfo getAuthorizeInfoFromCache(String identifier){
        ...
        cache = getCacheManager().getCache(AUTHORIZE_CACHE_NAME);
        Object o = cache.get(identifier);
        if(o == null){
            logger.fine("No data of [{"+identifier+"}] store in cache [{"+AUTHORIZE_CACHE_NAME+"}]!");
        }else {
            if(cache instanceof AuthorizeInfo){
                return (AuthorizeInfo) cache;
            }else {
    ...
    protected void doClearCache(Principal principal){
        ...
        if(isCacheEnabled() && getCacheManager() != null) {
            Cache<String, AuthorizeInfo> authorizeInfoCache = getCacheManager().getCache(AUTHORIZE_CACHE_NAME);
            if (authorizeInfoCache != null) {
