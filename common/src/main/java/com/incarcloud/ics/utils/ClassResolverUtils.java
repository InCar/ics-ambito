...
/**
 * 类解析工具类，可用于获取路径中特定类型的类，如某个类的所有实现类，或者标记了特定注解的类等等。借鉴ibatis的io包的ResolverUtil类，
 */
public class ClassResolverUtils {
	...
	public static <T> Set<Class<?>> findImplementations(Class<?> parent, String... packageNames) {
		...
		ClassResolverUtils cu = new ClassResolverUtils();
	 ...
	 * @param packageName the tableName of the package from which to start scanning for
	 *        classes, e.g. {@code net.sourceforge.stripes}
	 */
	public ClassResolverUtils find(Test test, String packageName) {
	...
	/**
	 * Converts a Java package tableName to a path that can be looked up with a call to
	 * {@link ClassLoader#getResources(String)}.
	 *
	 * @param packageName The Java package tableName to convert to a path
	 */
	...
	/**
	 * Add the class designated by the fully qualified class tableName provided to the set of
	 * resolved classes if and only if it is approved by the Test supplied.
	 *
	 * @param test the test used to determine if the class matches
	 * @param fqn the fully qualified tableName of a class
	 */
...
abstract class VFS {
	...
	/** Get a class by tableName. If the class is not found then return null. */
	...
	/**
	 * Get a method by tableName and parameter types. If the method is not found then return null.
	 *
	 * @param clazz The class to which the method belongs.
	 * @param methodName The tableName of the method.
	...
	protected URL findJarForResource(URL url) throws MalformedURLException {
		...
		try {
			...
			if (isJar(testUrl)) {
				...
				// File tableName might be URL-encoded
				if (!file.exists()) {
					try {
